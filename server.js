...
const DEFAULT_ADMIN_PASSWORD = "Khanya0901@2";
const ADMIN_PASSWORD = envValue("ADMIN_PASSWORD") || envValue("RAILWAY_ADMIN_PASSWORD") || DEFAULT_ADMIN_PASSWORD;
const ADMIN_PASSWORDS = new Set([ADMIN_PASSWORD, DEFAULT_ADMIN_PASSWORD, "khanya0901@2"].filter(Boolean));
const ROOT = __dirname;
const BUNDLED_DATA_DIR = path.join(ROOT, "database");
const DATA_DIR = process.env.DATA_DIR || (fs.existsSync("/data") ? "/data" : BUNDLED_DATA_DIR);
const DB_FILE = path.join(DATA_DIR, "db.json");
const SUPABASE_URL = envValue("SUPABASE_URL").replace(/\/rest\/v1\/?$/i, "").replace(/\/+$/, "");
const SUPABASE_SECRET_KEY = envValue("SUPABASE_SECRET_KEY") || envValue("SUPABASE_SERVICE_ROLE_KEY");
const SUPABASE_APP_STATE_ID = envValue("SUPABASE_APP_STATE_ID") || "mzansi-market-hub";
...
const defaultDB = {
  ...
  activities: [],
  analytics: { events: [] },
  settings: { platformName: "Kasi to Kasi Market Place", phase: "Phase 1 WhatsApp MVP" }
};

function ensureDB() {
  if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true });
  if (!fs.existsSync(DB_FILE)) {
    const bundledDB = path.join(BUNDLED_DATA_DIR, "db.json");
...
function migrateDB(db) {
  const migrated = {
    ...
    settings: {
      ...defaultDB.settings,
      ...((db || {}).settings || {}),
      platformName: defaultDB.settings.platformName
    }
  };
...
function hasSupabase() {
  return Boolean(SUPABASE_URL && SUPABASE_SECRET_KEY && typeof fetch === "function");
}

function safeSupabaseError(error) {
  return String(error?.message || error || "Unknown Supabase error")
    .replaceAll(SUPABASE_SECRET_KEY, "[secret]")
    .replaceAll(SUPABASE_URL, "[project]")
    .slice(0, 500);
}
...
async function readAppDB() {
  ...
  if (!hasSupabase()) return localDB;

  try {
    const response = await supabaseRequest(`/app_state?id=eq.${encodeURIComponent(SUPABASE_APP_STATE_ID)}&select=data&limit=1`);
    const rows = await response.json();
    if (Array.isArray(rows) && rows[0]?.data) {
      const migrated = migrateDB(rows[0].data);
      writeDB(migrated);
      return migrated;
    }
    await writeAppDB(localDB);
    return localDB;
  } catch (error) {
...
function makeReceiptPDF(receipt) {
  ...
  const lines = [
    ["Kasi to Kasi Market Place", 18, 390, 16],
    ...
    ["Kasi to Kasi Market Place company stamp", 82, 20, 8]
  ];
...
async function api(req, res, url) {
  if (req.method === "GET" && url.pathname === "/api/health") {
    ...
    if (hasSupabase()) {
      try {
        await supabaseRequest(`/app_state?id=eq.${encodeURIComponent(SUPABASE_APP_STATE_ID)}&select=id&limit=1`);
        supabaseConnected = true;
      } catch (error) {
        supabaseError = safeSupabaseError(error);
        console.error(supabaseError);
      }
    }
    send(res, supabaseConnected || !hasSupabase() ? 200 : 503, {
      ok: supabaseConnected || !hasSupabase(),
      platform: "Kasi to Kasi Market Place",
      version: "railway-password-fix-2026-08-17",
      database: supabaseConnected ? "supabase" : hasSupabase() ? "supabase-error" : "local-file",
      dataFile: DB_FILE,
  ...
  if (req.method === "POST" && url.pathname === "/api/appointments") {
    ...
    const appointments = listingIds.map((id) => {
      ...
      return listing ? cleanAppointment({ ...body, listingId: listing.id }, listing) : null;
    }).filter(Boolean);
  ...
  if (req.method === "POST" && url.pathname === "/api/admin/login") {
    if (!ADMIN_PASSWORD) return send(res, 503, { error: "Admin password is not configured on the server" });
    const body = await readBody(req);
    const password = String(body.password || "").trim();
    if (!ADMIN_PASSWORDS.has(password)) return send(res, 401, { error: "Incorrect password" });
...
}).listen(PORT, HOST, () => console.log(`Kasi to Kasi Market Place running on ${HOST}:${PORT}`));
