...
async function api(req, res, url) {
  ...
  if (req.method === "POST" && url.pathname === "/api/admin/login") {
    ...
    const body = await readBody(req);
    if (String(body.password || "").trim() !== ADMIN_PASSWORD) return send(res, 401, { error: "Incorrect password" });
...
ensureDB();
if (!ADMIN_PASSWORD) console.warn("ADMIN_PASSWORD is not configured. Admin login will remain disabled.");
...
}).listen(PORT, () => console.log(`Kasi to Kasi Market Place running at http://localhost:${PORT}`));
